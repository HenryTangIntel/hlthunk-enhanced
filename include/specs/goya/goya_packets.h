...
/* IMPORTANT NOTE:
 * goya_dma_direction enum MUST match exactly enum "hl_goya_dma_direction" in misc/habanalabs.h.
...
/* IMPORTANT NOTE:
 * User-space code *must* write the packets in LE format, irrespective of
 * the host architecture. In particular, user-space code generating
 * packets need to include the <endian.h> header file and perform any
 * needed conversion with the htoleXX() macros (See tests/hlthunk_tests_goya.c
 * as an example).
 *
 * This is necessary since the H/W processing these packets is LE
 * architecture. The habanalabs Linux Kernel Driver will perform any
 * necessary conversion to CPU (and back to LE) for any needed packet
 * introspection, validation, and patching, before sending off to the H/W.
 *
 * Also note that we still need separate bit-field mappings depending on
 * the architecture. This isn't due to endianness per-se. It's due to the
 * way the compiler packs the bits. On LE, bits are packed from right to
 * left. On BE, bits are packed from left to right. Thus, in order to
 * maintain the same bit order, the mappings need to be mirror images of
 * each other.
 */
struct packet_nop {
	__u32 reserved;
