...
def write_only(pci_lib, fd):
    val = ctypes.c_uint64(0xffffffffffffffff)
    step = 4
    ...
    while (1):
        ...
        for i in range(np.uint64(0x7FF8000000), np.uint64(0x8000000000), step):
            pci_lib.write_64bit(fd, ctypes.c_uint64(i), val)
...
def write_and_read(pci_lib, fd):
    ...
    while (1):
        ...
        for i in range(np.uint64(0x7FF8000000), np.uint64(0x8000000000), step):
            pci_lib.write_64bit(fd, ctypes.c_uint64(i), val)
            read_val = pci_lib.read_64bit(fd, ctypes.c_uint64(i))
