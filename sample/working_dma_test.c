...
// Ensure that config_iterator.c is included in the build process.
    char error_msg[256];
};
...
struct test_result test_host_to_device(int fd) {
    ...
    rc = hltests_dma_transfer(fd, 
                             ...
                             (uint64_t)device_addr,  // dst address (device)
                             TEST_SIZE,
...
struct test_result test_device_to_host(int fd) {
    ...
    rc = hltests_dma_transfer(fd,
                             ...
                             EB_FALSE, // eb
                             MB_TRUE,  // mb
                             (uint64_t)device_addr,  // src address (device)
...
struct test_result test_device_to_device(int fd) {
    ...
    rc = hltests_dma_transfer(fd,
                             ...
                             EB_FALSE, // eb
                             MB_TRUE,  // mb
                             (uint64_t)device_addr1,  // src address (device)
                             (uint64_t)device_addr2,  // dst address (device)
                             TEST_SIZE,
